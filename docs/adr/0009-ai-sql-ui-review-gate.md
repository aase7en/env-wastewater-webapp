...
**Resolved 2026-07-21 (AISQL-phi-filter)**: the dynamic PHI filter is now in `lib/admin/ai-sql.ts`. `buildSchemaContext()` calls `loadPhiDenySet()` (reads `core.ai_scope` for `is_enabled=true AND patient_safe=false`) → `filterPhiTables()` drops matching names before any row count is fetched. The provider never sees `core.app_user` or `core.personnel` in the catalog. Defensive fallback: if `ai_scope` is unreadable, the call returns an empty deny-set so the feature still works (the execute-path whitelist + review-gate remain the structural PHI backstop). The DBA-8 WO's hardcoded env-only list is preserved as `SCHEMA_CONTEXT_TABLES` and now also includes `core.app_user` (filtered out at runtime).
