...
**Resolved 2026-07-21 (AISQL-phi-filter)**: the dynamic PHI filter is now in `lib/admin/ai-sql.ts`. `buildSchemaContext()` calls `loadPhiDenySet()` (reads `core.ai_scope` for `is_enabled=true AND patient_safe=false`) → `filterPhiTables()` drops matching names before any row count is fetched. The provider never sees `core.app_user` or `core.personnel` in the catalog. Defensive fallback is **fail-CLOSED**: if `ai_scope` is unreadable, the call returns `STATIC_PHI_DENY` (`{core.app_user, core.personnel}`) — an outage can never widen what the provider sees (REVIEW-9 fix; the previous fail-open empty-set return was rejected as a PHI-boundary violation under the GLM-cloud / Chinese-law constraint). The DBA-8 WO's hardcoded env-only list is preserved as `SCHEMA_CONTEXT_TABLES` and now also includes `core.app_user` (filtered out at runtime).

**Updated 2026-08-02 (I2 — ai-chat fail-closed parity)**: `lib/admin/ai-chat.ts applyPhiFilter()` now mirrors the same fail-CLOSED contract. The chat path previously failed open on an `ai_scope` read error (returned `{blocked:false}`, sending the question verbatim to the provider) — contradicting the hardened `ai-sql.ts` sibling. Both paths now fall back to `STATIC_PHI_DENY` on outage. Same fail-closed rule applies to both the NL→SQL path and the chat path.

### 3. Audit viewer reuses `public.audit_log` — no new SQL

The viewer is a read-only admin page querying `supabase.from("audit_log")`. The `security_invoker=on` façade + `audit_log_admin_all` policy already make it admin-only at the DB layer — a non-admin gets `401/42501`, and the route is `RequireAuth requireAdmin` on top. No migration, no new RPC. Columns: `id, actor, action, table_name, row_id, changed_at, old_data, new_data` (verified against `reports/schema-snapshot-live.md` 2026-07-21: `id` is `bigint`, time column is `changed_at` — *not* `created_at`/`uuid` as the original WO draft guessed). Basic filters only (date range, `action`, `table_name`); `old_data`/`new_data` shown as expandable JSON. No PHI risk — audit rows carry env-domain diffs, and `actor` is a UUID.
