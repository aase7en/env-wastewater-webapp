...
- **ACTIVE `GARBAGE-CORE-001` / CLAIMED** — Core/data-contract lane on `fix/garbage-core-001`, worktree `A:\\GitHub\\envww-garbage-core-001`, base `origin/main@196d023b9f2f2b3bfb1837ad2ad1bd6d713fde84`. Owner GLM-5.3 MAX; lead/review/merge owner GPT-5.6 Sol. Canonical field is `segregation_type`; legacy `waste_type` is read compatibility only. Live table is currently empty, so no backfill/cleanup is authorized. Carbon must not map chemical/missing/unknown to general. Contract: `docs/work-orders/GARBAGE-CORE-001.md`.
...
- Next safe action: GLM-5.3 MAX executes `GARBAGE-CORE-001` from the frozen activation checkpoint, establishes RED/baseline evidence, implements only the bounded Core contract, verifies, pushes/opens PR, writes result to the shared packet, and stops at `REVIEW_REQUESTED`.
