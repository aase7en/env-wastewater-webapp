...
- **NEXT GPT MAX lane: `ENV-OPS-001` READY AFTER THIS CLOSEOUT** - Command Center and Flow dependencies are satisfied. First step is requirement/contract inventory for incidents, repair requests, inspections, thresholds, responsible party and evidence semantics before any UI implementation. Building/Repair remains Ultra-owned/protected and must not be used as an editable dependency.
