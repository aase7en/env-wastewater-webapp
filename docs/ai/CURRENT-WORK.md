  ...
  "coordination_registry": {
    ...
    "claims": [
      ...
      },
      {
        ...
        "one_next_safe_action": "Repair the three P1 findings from exact-head review at f7ea87d942cfcc5118d0fde63037cd4de89a7bc4: close Git shell bypasses, bind UNKNOWN hook effects to lifecycle recovery, and make Kilo receipts trusted and durable. Revalidate, publish a CHANGES_REQUIRED checkpoint, then request fresh exact-SHA review."
      }
    ]
  }
}
...
- **ENV-AUTONOMY-001 / CHANGES_REQUIRED / BOOTSTRAP_CONTROL** — PR #91 exact-head review at `f7ea87d942cfcc5118d0fde63037cd4de89a7bc4` found three P1 issues: Git shell commands classified as read-only despite mutation options; checkpoint/recovery ignoring unresolved `UNKNOWN` hook observations; and Kilo receipt transitions trusting caller-supplied verification without persisted claim-bound ingestion. Hosted `scripts` and `notify` checks passed at that head, which is now stale for review. Repair within the registered scope, keep readiness disabled, publish a fresh checkpoint, then request exact-head re-review. C1's four files and production/frontend/schema/data paths remain untouched.
