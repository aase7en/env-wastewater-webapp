...
- No active mutable production work remains from `ENV-MOBILE-006-GARBAGE`; `ENV-MOBILE-001` through `ENV-MOBILE-006` are complete/merged/deployed history. The next mobile lane must be selected from Food / Safety / Building by a fresh read-only source/data/control-density audit before any mutable claim.
...
- Next safe action: run a bounded read-only audit of Food / Safety / Building against current main, compare field frequency/control density plus source/data-contract risk, then activate exactly one next mobile WO with disjoint ownership; do not fold business-rule or schema changes into the visual slice.
