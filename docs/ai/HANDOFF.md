...
- Status: `CLAIMED`; implementation owner GLM-5.3 MAX, lead/review/merge owner GPT-5.6 Sol.
...
- Contract: `docs/work-orders/GARBAGE-CORE-001.md`; activation HEAD is frozen after this docs checkpoint.
...
- One next safe action: GLM-5.3 MAX reads SSoT + WO, verifies `SAFE_TO_MUTATE`, establishes RED/baseline, implements/tests/pushes/opens PR, writes result to the same external packet, then stops at `REVIEW_REQUESTED`.
