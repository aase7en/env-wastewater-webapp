...
**Active bounded slice:** `ENV-MOBILE-007` — Safety. Fresh exact-main read-only selection audit chose Safety from Food / Safety / Building because it has the highest current control density (12 visible controls), monthly/next-due field cadence, and a direct single-domain CRUD seam. Food reagent-stock coupling and Building repair-request coupling remain out of scope/read-only. This slice is visual/mobile only: one-column phone composition, labels/toggles, 44px actions, contained history, required-date recovery, failure/retry preservation, and realistic Playwright evidence while preserving all Safety Core/import/schema/RLS/alert semantics. Contract: `docs/work-orders/ENV-MOBILE-007-SAFETY.md`.
