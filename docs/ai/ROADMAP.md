...
**Next bounded activation candidate:** `ENV-MOBILE-008` — Food. Activate only from fresh current main after this closeout reaches main. Use the proven mobile convergence pattern but keep Food Core/import/schema/RLS/reagent-trigger/shared UI read-only. Establish a fresh RED baseline rather than assuming the Safety/Garbage geometry. Building remains read-only/`DECISION_REQUIRED` until the `repair_needed` → `core.repair_request` contract is settled.
