...
**Active bounded slice:** `ENV-MOBILE-008` — Food, now `REVIEW_REQUESTED`. Dedicated branch/worktree is based on exact main `b05a25e182ddd648414f2d1d35be79ef67776e15`. Truthful RED 6 FAIL / 3 PASS closed to focused Playwright 9/9 PASS; full Vitest 248/248, TypeScript, lint (12 pre-existing warnings / 0 errors), diff-check and post-edit production build all pass. Full Playwright is locally transport-blocked by repeated connector HTTP 502 and must pass at exact PR head in GitHub CI before merge. `FOOD-CORE-001` remains CLOSED/LIVE VERIFIED; Food Core/import/schema/RLS/reagent-trigger/shared UI stay read-only. Building remains read-only/`DECISION_REQUIRED` until the `repair_needed` → `core.repair_request` contract is settled.
