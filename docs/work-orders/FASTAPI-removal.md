...
Status: done (Approach C executed by fable5, 2026-07-19 — commit hash ดู Checkpoint log)
...
- [x] Fable5 verify scripts ที่จะเหลือทำงานได้จริง (apply_migration_api คือ
      สคริปต์สำคัญที่ใช้ทุก migration) → ผล 2026-07-19: split_sql 8/8;
      introspect + apply_migration ผ่านโดย **ล้าง SUPABASE_ACCESS_TOKEN ใน env
      ก่อนรัน** (บังคับเดิน stub → `_env.py` → Drive secrets เต็มเส้น) —
      introspect เขียน snapshot สำเร็จ, no-op `SELECT 1;` migration = 1/1 OK;
      `git grep` ไม่เหลือ `app.` refs ใน scripts/.github; `npm run build` ผ่าน
...
- [2026-07-19] zcode (GLM sweep): inventory + dependency graph เสร็จ.
  ...
  รอ Fable5 ตัดสินใจ.
- [2026-07-19] fable5: audit + **executed Approach C** ครบทุก step —
  archive branch `archive/fastapi-backend` (= `0841078`) push แล้ว;
  `scripts/_env.py` ใหม่ (stdlib); 2 scripts ported; `app/` + `tests/` +
  `introspect_schema.py` ลบ; pyproject 0.2.0 (dep เหลือ httpx) + uv.lock
  regen; test.yml → split_sql regression; docs 4 ไฟล์ + `.env.example`
  อัปเดต; `reports/schema-snapshot-live.md` refresh (ของเดิม stale ก่อน
  SCHEMA-1..4). Verify ครบตาม Step 8 (ดู Acceptance gate).
  **Nit follow-up (ไม่ block)**: `introspect_schema_api.py::SCHEMAS` ยัง
  ครอบแค่ `core/carbon/wastewater` — v2 มี 11 domain schemas แล้ว; ขยาย
  tuple = chunk เล็ก cheap-ok.
  Commit hash: `c6fc72a` (claim `0841078`; archive branch ชี้ `0841078`).
